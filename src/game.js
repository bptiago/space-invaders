...
const playerBulletController = new BulletController(canvas, 3, "green", true);
const enemyBulletController = new BulletController(canvas, 8, "yellow", false);
...
function displayGameOver(ctx) {
    if (isGameOver) {
        ...
        ctx.font = "30px Arial";
        ctx.fillText(
            `Inimigos abatidos: ${enemiesDefeated}`,
            canvas.width / 3.5,
            canvas.height / 1.5 + 50
        );
    }
}
